...
async function handleRequest(req, res) {
  ...
  try {
    ...
    // Public assets use root-relative browser URLs in both Vite and the
    // standalone dev server (for example /brand/sfc-mark.svg).
    const publicRoot = path.resolve(__dirname, 'public');
    const publicCandidate = path.resolve(publicRoot, webPath.replace(/^\/+/, ''));
    if (
      !fs.existsSync(filePath)
      && publicCandidate.startsWith(publicRoot + path.sep)
